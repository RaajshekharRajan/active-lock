...
unsafe extern "system" fn keyboard_hook_proc(
    ...
    if code >= 0 {
        ...
        let alt_down = kb.flags.0 & 0x20 != 0; // LLKHF_ALTDOWN

        let block = match vk {
            0x5B | 0x5C => true,                            // VK_LWIN / VK_RWIN
            0x09 if alt_down => true,                       // Alt+Tab
            0x73 if alt_down => true,                       // Alt+F4
            0x1B if unsafe { GetAsyncKeyState(0x11) } < 0 => true, // Ctrl+Esc
            _ => false,
        };
    ...
    unsafe { CallNextHookEx(Some(hook), code, w_param, l_param) }
}
...
impl WindowsLock {
    ...
    fn install_keyboard_hook(&mut self) -> Result<(), String> {
        unsafe {
            ...
            let hook = SetWindowsHookExW(WH_KEYBOARD_LL, Some(keyboard_hook_proc), Some(h_module.into()), 0)
        ...
        unsafe {
            ...
            if RegOpenKeyExW(HKEY_CURRENT_USER, subkey, 0, KEY_READ, &mut key).is_ok() {
                ...
                if RegQueryValueExW(
                    ...
                    None,
                    None,
                    Some(std::slice::from_raw_parts_mut(&mut val as *mut u32 as *mut u8, size as usize)),
                    Some(&mut size),
                )
                .is_ok()
                    && val == 1
            ...
            let _ = RegCloseKey(key);
            res.map_err(|e| format!("RegSetValueExW: {e}"))?;
        }
    ...
    fn enable_task_manager(&self) {
        ...
        unsafe {
            let mut key = HKEY::default();
            if RegOpenKeyExW(HKEY_CURRENT_USER, subkey, 0, KEY_WRITE, &mut key).is_ok() {
